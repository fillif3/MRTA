...
public class collision {
	...
	private static int maxNeighbour(int[][] map, int i, int j) {
		// TODO Auto-generated method stub
		int max =0;
		if (i>0) max = map[i-1][j];
		if (j>0)
		{
	...
	public static int[][] collisionMap(PoseSteering[] path1, double[][] polygon1,PoseSteering[] path2, double[][] polygon2, double safe1,double dangerous1,double safe2,double dangerous2)
	{
		...
		for (int i = 0;i<(path1.length-1);i++)
		{
			...
			for (int j = 0;j<(path2.length-1);j++)
			{
				if (map[i/10][j/10]==0) 
				{
					...
					map[i/10][j/10]=detectCollision(nodes1,path1[i].getPose(),safe1,dangerous1,nodes2,path2[j].getPose(),safe2,dangerous2)? 1 : 0;
				}
			}
